...
public class Comp130_HW3_Fall19 extends GraphicsProgram {
	...
	double r1 = rgen.nextDouble(MIN_SPEED, MAX_SPEED);
	double r2 = rgen.nextDouble(MIN_SPEED, MAX_SPEED);
	int tBlue = 0;
	int tRed = 0;
	int totalBlue = 0;
	int totalRed = 0;
	...
	public void run(){
		...
		while(!(isCollide(blueCar, redCar))) { //loop goes until blue and red collides
				...
				if(redCar.getX() + CAR_LENGHT <= 0) { //do certain things like recreating while redCar is outside of bounds
					...
					add(redCar);
					double rSpeed1 = rgen.nextDouble(MIN_SPEED, MAX_SPEED); //generating random speed
					r1 = rSpeed1;
				}
				if(blueCar.getY() >= SCREEN_HEIGHT) { //do certain things like recreating while blueCar is outside of bounds
					...
					add(blueCar);
					double rSpeed2 = rgen.nextDouble(MIN_SPEED, MAX_SPEED); //generating random speed
					r2 = rSpeed2;
				}
	...
	private void isRedLightFine() { //checks the conditions for red light
			...
			if(vLight.getFillColor()==Color.RED && blueCar.getY()+blueCar.getHeight()>=verticalLine && !cRF1 && !bF1) {
			...
			if(hLight.getFillColor()==Color.RED && redCar.getX()<=horizontalLine && !cRF2 && !bF2) {
